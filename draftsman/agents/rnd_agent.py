...
class RNDAgent(DQNAgent):
    ...
    def update(
        ...
        with torch.no_grad():
            ...
            # TODO might need to normalize the error
            rnd_error = self.rnd_net(observations) - self.rnd_target_net(observations)
            rnd_error = F.normalize(rnd_error, dim=0)
            rnd_error = torch.norm(rnd_error, dim=-1)
