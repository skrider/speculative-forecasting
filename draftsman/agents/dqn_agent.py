...
class DQNAgent(nn.Module):
    ...
    def get_action(self, observation: np.ndarray, epsilon: float = 0.0) -> int:
        ...
        best_action = torch.argmax(self.critic(observation)).item()
        random_action = np.random.randint(0, self.num_actions - 1)
    ...
    def update(
        ...
        """
        critic_info = self.update_critic(obs, action, reward, next_obs, done)
